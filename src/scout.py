...
SCOUT_PROMPT = (
    "You are scouting a {window_duration:.1f}-second window of gameplay video. "
    "Frames are sampled in chronological order.\n"
    "\n"
    "Your job: identify every notable in-game engagement in this window. A separate "
    "detail pass will score and refine each region you find. Under-flagging is much "
    "worse than over-flagging — when in doubt, INCLUDE the region.\n"
    "\n"
    "Treat the following as flag-worthy regions:\n"
    "  - Combat encounters: firefights, multi-kills, clutches, aces, big damage trades.\n"
    "  - Boss fights and mini-boss encounters: flag the WHOLE fight as a single region,\n"
    "    from the first engagement to the resolution. Do not split a boss fight into\n"
    "    multiple regions just because it lasts a long time.\n"
    "  - Sustained engagements: extended firefights, chase sequences, multi-stage events.\n"
    "    The whole engagement is one region.\n"
    "  - Movement showcases: bhop, parkour, sick aim, no-scopes, wall-bangs.\n"
    "  - Glitches, fails, surprising deaths, jumpscares.\n"
    "  - Emotional reactions, comeback wins, satisfying finishers.\n"
    "\n"
    "If a region is clearly ongoing at the window boundary, extend it to the boundary — "
    "don't trim because the window cuts off. A second window will see the rest and a "
    "merge step will stitch them.\n"
    "\n"
    "For each flagged region, return start_sec and end_sec relative to THIS window's "
    "start (0 = first frame), plus the closest type tag:\n"
